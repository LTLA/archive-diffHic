...
For full compatibility, users should include the alignment positions and lengths as \code{xxx.pos} and \code{xxx.len} for both the anchor and target reads (replacing \code{xxx} with \code{anchor} or \code{target}).
...
This generates a \code{DIList} object that contains the relevant count and coordinate information.
...
% Smaller bins also provide some protection against edge effects, when an interaction is not 
% centered within a larger bin.
...
Each row of the returned \code{DIList} contains the marginal read counts for a single genomic bin.
...
The \code{count.keep} vector is then used to subset the \code{DIList}.
...
This favours selection of longer-range interactions, compared to the direct filter.
...
Peaks in the interaction space refer to those bin pairs that have substantially more reads than their neighbours.
The \code{enrichedPairs} function computes an enrichment value for each bin pair, defined as the log-fold change of its abundance against that of its neighbours.
...
More specifically, the function partitions the neighbourhood into several different regions around the target bin pair.
The size of the neighbourhood is set by \code{flank}, and is defined in terms of bins (actual distance of $\sim$5 Mbp, in this case).
...
The neighbourhood regions are designed to capture high-intensity structural features like looping domains, TADs or banding patterns \citep{rao2014kilobase}.
...
This is a modest threshold that errs on the side of caution, as weak peaks may be DB and should be retained for testing.
In practice, filtering of enrichment values should be combined with filtering on absolute abundances.
This eliminates low-abundance bin pairs with high enrichment values, e.g., when the neighbourhood is empty.
...
Briefly, the colour intensity of each ``pixel'' is proportional to the number of read pairs between the corresponding loci on each axis.
...
#	first=resize(anchors(data[chosen]), fix="center", width=10e6),
#	second=resize(targets(data[chosen]), fix="center", width=10e6))
a.list <- GRanges(c("chr11", "chr4"), IRanges(
			c(80999224, 153002112),
			c(81998782, 154006051)))
t.list <- GRanges(c("chr11", "chr4"), IRanges(
			c(66001477, 130998845),
			c(67011476, 131997373)))
capped <- c(50, 50)
par(mfrow=c(1,2))
for (chosen in 1:length(a.list)) { 
	if (!suppressWarnings(any(overlapsAny(anchors(data[peak.keep]), a.list[chosen], type="equal") & 
			overlapsAny(targets(data[peak.keep]), t.list[chosen], type="equal")))) { stop("can't find example for peak-calling") }
	plotPlaid(input[3], param=mm.param, width=2e5, max.count=capped[chosen], 
			first.region=resize(a.list[chosen], fix="center", width=10e6),
			second.region=resize(t.list[chosen], fix="center", width=10e6),
...
This is because each peak, by definition, should be isolated in its neighbourhood.
Larger proportions suggest that more aggressive filtering on the enrichment score is required, e.g., in the second example. 

<<>>=
neighbourhood <- (2*flank.width + 1) * exptData(data)$width
boxed <- boxPairs(data[peak.keep], reference=neighbourhood)
...
boxed <- boxPairs(data[peak.keep2], reference=neighbourhood)
out <- tabulate(tabulate(boxed$indices[[1]]))
setNames(round(c(out[1:5], sum(out[5:length(out)]))/sum(out)*100, 1), c(1:5, ">5"))
@
...
However, as previously mentioned, bin pairs near the diagonal may exhibit irregular behaviour relative to the rest of the interaction space.
...
<<>>=
redist <- getDistance(data) 
neardiag <- is.na(redist) | redist > 1.5e6
nb.off <- matrix(0, nrow=nrow(data), ncol=ncol(data))
nb.off[neardiag] <- normalize(data[neardiag,], type="loess")
nb.off[!neardiag] <- normalize(data[!neardiag,], type="loess")
@
...
For example, a global increase in the compaction of the genome would manifest as an upward trend in the MA plot, as low-abundance distal interactions are weakened in favour of high-abundance short-range interactions.
...
<<results=hide,echo=FALSE>>=
<<setup>>
if (as.character(seqnames(ax.s[chosen]))!="chr11" || start(ax.s[chosen])!=30001298 || end(ax.s[chosen])!=31000010 ||
    as.character(seqnames(tx.s[chosen]))!="chr11" || start(tx.s[chosen])!=29000942 || end(tx.s[chosen])!=30001301) {
...
The \code{max.count} value controls the relative scale of the colours. 
Any pixel with a larger count will be set at the maximum colour intensity.
This ensures that a few high-count regions do not dominate the plot.
A smaller \code{max.count} is necessary for smaller libraries so that the intensity of the colours is comparable.
The actual colour can be set by specifying \code{col}.
...
<<results=hide,echo=FALSE>>=
<<setup2>>
if (as.character(seqnames(ax.d[chosen]))!="chr3" || start(ax.d[chosen])!=151001261 || end(ax.d[chosen])!=152002835 ||
    as.character(seqnames(tx.d[chosen]))!="chr3" || start(tx.d[chosen])!=145998892 || end(tx.d[chosen])!=147005847) {
...
<<echo=FALSE,results=hide>>=
if (as.character(seqnames(ax.s[chosen]))!="chr2" || start(ax.s[chosen])!=136998216 || end(ax.s[chosen])!=138001392 ||
    as.character(seqnames(tx.s[chosen]))!="chr2" || start(tx.s[chosen])!=136001779 || end(tx.s[chosen])!=136998219) {
...
This can be achieved using the \code{plotDI} function, which assigns colours to bin pairs according to the size and direction of the log-fold change.
...
<<results=hide,echo=FALSE>>=
if (as.character(seqnames(ax.d[chosen]))!="chr16" || start(ax.d[chosen])!=70000341 || end(ax.d[chosen])!=70999295 ||
    as.character(seqnames(tx.d[chosen]))!="chr16" || start(tx.d[chosen])!=65999261 || end(tx.d[chosen])!=67001066) {
...
% This is a pain, though we could do it by converting the DIList to a full matrix, and running PCA on the matrix for each chromosome.

Both \code{rotDI} and \code{plotDI} will invisibly return another function that maps log-fold changes to colours.
This can be used to generate a custom colour bar, as shown below.
A similar function that maps counts to colours is also returned by \code{plotPlaid} and \code{rotPlaid}.
...
Such domains are proposed to control genomic behaviour by limiting the scope for interactions and restraining the spread of chromatin marks \citep{nora2013segmental}.
...
For each bin, the total number of read pairs mapped between that bin and its closest \code{span} upstream neighbours is counted.
This is repeated for its closest \code{span} downstream neighbours.
...
rect(start(finder[onchr1]), 0, end(finder[onchr1]), 1e5,
   col=colours[all.states], border=NA)
@
